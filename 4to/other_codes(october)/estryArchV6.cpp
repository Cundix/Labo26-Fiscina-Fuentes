...
struct juego_t
{
    string nombre[LARGO];      //Nombre del juego
    string version[LARGO];     //Versión del tipo 1.47.12
...
int main (void)
{
    ...
    //vector <string> opcionesM = {"Agregar Juego", "Mostrar Juegos", "Guardar Juegos en un archivo", "Salir"};

    vector <string> opcionesM;
    cargarVector(opcionesM, idioma)

    while (!salgo)
    {
        cout << opcionesM[0] << endl;
        cout << "\t" << AGREGAR << " - " << opcionesM[1] << endl;
        cout << "\t" << LISTAR << " - " << opcionesM[2] << endl;
        cout << "\t" << GUARDAR << " - " << opcionesM[3] << endl;
        cout << "\t" << SALIR << " - " << opcionesM[4] << endl;
        cout << "\t---> ";
        cin >> opcion; 
